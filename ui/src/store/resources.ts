...
export const ResourceStore = types
	...
	.actions((self) => ({
		load: flow(function* () {
			const catalogItem = new Set();
			const kindItem = new Set();
			try {
				...
				[...Array.from(kindItem)].forEach((c) => {
					self.kindStore.add({ name: c });
				});

				[...Array.from(catalogItem)].forEach((c) => {
					self.catalogStore.add({ name: c });
				});
			} catch (err) {
	...
	.views((self) => ({
		get list() {
			...
			const filterResources = resources.filter(
				(r: any) =>
					(tag.size > 0 ? r.tags.some((t: any) => tag.has(t.name)) : true) &&
