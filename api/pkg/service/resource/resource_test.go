...
func TestQuery_ByNameTagsAndMultipleType(t *testing.T) {
	...
	resourceSvc := New(tc)
	payload := &resource.QueryPayload{Name: "build", Kinds: []string{"task", "pipeline"}, Tags: []string{"atag", "ztag"}, Exact: false, Limit: 100}
	all, err := resourceSvc.Query(context.Background(), payload)
	assert.NoError(t, err)
	assert.Equal(t, 2, len(all))
}
...
func TestQuery_ByExactNameAndMultipleType(t *testing.T) {
	...
	resourceSvc := New(tc)
	payload := &resource.QueryPayload{Name: "buildah", Kinds: []string{"task", "pipeline"}, Exact: true, Limit: 100}
	all, err := resourceSvc.Query(context.Background(), payload)
	assert.NoError(t, err)
	assert.Equal(t, 1, len(all))
}
...
func TestQuery_ExactNameNotFoundError(t *testing.T) {
	...
	resourceSvc := New(tc)
	payload := &resource.QueryPayload{Name: "build", Kinds: []string{}, Exact: true, Limit: 100}
