...
// CLI flags.
func BuildQueryPayload(resourceQueryName string, resourceQueryKinds string, resourceQueryTags string, resourceQueryLimit string, resourceQueryExact string) (*resource.QueryPayload, error) {
	var err error
	var name string
	{
	...
	var exact bool
	{
		if resourceQueryExact != "" {
			exact, err = strconv.ParseBool(resourceQueryExact)
			if err != nil {
				return nil, fmt.Errorf("invalid value for exact, must be BOOL")
			}
		}
	}
